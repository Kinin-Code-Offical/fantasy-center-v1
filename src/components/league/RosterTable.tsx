...
export default function RosterTable({ players, leagueId }: RosterTableProps) {
    ...
    return (
        <div className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {players.map((player) => (
                    <Link href={`/player/${player.id}?leagueId=${leagueId}`} key={player.id} className="block">
                        <div
                            className="group relative bg-gray-900/80 border border-white/10 rounded-lg p-4 hover:border-green-500 hover:shadow-[0_0_15px_rgba(74,222,128,0.3)] transition-all duration-300 overflow-hidden h-full"
                        >
