        ...
        <div className="rounded-xl border border-white/10 bg-black/40 p-6 relative overflow-hidden">
            ...
            <h3 className="mb-6 text-sm font-bold text-gray-400 uppercase tracking-widest flex items-center justify-between relative z-10">
                <span className="flex items-center gap-2">
                    <span className="w-1 h-4 bg-green-500 rounded-full shadow-sm shadow-green-500/50"></span>
                    League Standings
                </span>
                ...
                {teams.map((team) => {
                    ...
                    return (
                        <Link
                            ...
                            scroll={false}
                            className={`group flex items-center justify-between p-3 rounded border transition-colors duration-200 ${isActive
                                    ? "bg-green-900/30 border-green-400 shadow-sm shadow-green-500/20 translate-x-1"
                                    : team.isUserTeam
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className={`flex h-6 w-6 items-center justify-center rounded text-[10px] font-bold font-mono border ${isActive || team.isUserTeam
                                        ? "bg-green-500 text-black border-green-400 shadow-sm shadow-green-500/30"
                                        : "bg-black text-gray-500 border-white/10"
                                    }`}>
                                    {team.rank || "-"}
                                </div>
                                <div className="flex flex-col">
                                    <span className={`text-xs font-bold uppercase tracking-wide ${isActive || team.isUserTeam ? "text-green-400" : "text-gray-300 group-hover:text-white"
                                        }`}>
                                        {team.name}
                                    </span>
                                    {team.isUserTeam && (
